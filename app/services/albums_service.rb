class AlbumsService
  ...
  def list_album(order_by, offset, limit, filter)
    albums = parse_all_albums(HTTParty.get("#{endpoint}albums/"))
    list_new = order_array(apply_limit(apply_offset(
                                         apply_filter(albums, filter), offset
                                       ), limit), order_by)
  ...
  def apply_filter(list, value, result = [])
    list.each do |elem|
      result << elem if elem['title'].include? value
    end
    result
  end
end
